...
function ChallengesCompletedRanking() {
        ...
        const fetchRanking = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log('Challenges Completed Ranking:', data);
                    setRanking(data);
                } else {
    ...
    const maxChallenges = ranking.length > 0 ? ranking[0].completedChallenges : 1;
    ...
    const handleExportExcel = () => {
        const data = filteredRanking.map((user, index) => ({
            ...
            "Số Challenge hoàn thành": user.completedChallenges,
        }));

        const ws = XLSX.utils.json_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Challenge hoàn thành");

        const excelBuffer = XLSX.write(wb, { bookType: "xlsx", type: "array" });
        saveAs(
            new Blob([excelBuffer], { type: "application/octet-stream" }),
            "bang_xep_hang_challenge_hoan_thanh.xlsx"
        );
    };
    ...
    return (
        <>
            <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h4 className="mb-1">✅ Bảng xếp hạng theo Thách thức hoàn thành</h4>
                    <p className="text-muted mb-0">Những người hoàn thành nhiều thử thách nhất</p>
                </div>
                    ...
                    <Table hover responsive style={{ background: "transparent" }}>
                        <thead>
                            <tr style={{ backgroundColor: '#f8f9fa' }}>
                                ...
                                <th className="text-center py-3" style={{ width: 180 }}>Challenge hoàn thành</th>
                            ...
                            {currentPageData.length === 0 ? (
                                ...
                                currentPageData.map((user, index) => {
                                    ...
                                    return (
                                        <tr
                                            key={user.userID}
                                            style={realIdx === 0 ? { background: "#f0f8ff" } : {}}
                                        >
                                            ...
                                            <td className="text-center py-3">
                                                <div className="d-flex align-items-center justify-content-center gap-2">
                                                    <FaCheckCircle color="#17a2b8" size={18} />
                                                    <span className="fw-bold" style={{ fontSize: 18, color: "#17a2b8" }}>
                                                        {user.completedChallenges}
                                                    </span>
                                                    <span className="text-muted">thử thách</span>
                                                </div>
                                            </td>
                                            <td className="text-center py-3">
                                                <ProgressBar
                                                    now={Math.min(100, (user.completedChallenges / maxChallenges) * 100)}
                                                    variant="info"
                                                    style={{ height: 8, minWidth: 120 }}
                                                />
                                                <small className="text-muted mt-1 d-block">
                                                    {((user.completedChallenges / maxChallenges) * 100).toFixed(1)}%
                                                </small>
                                            </td>
                                        </tr>
                                    );
...
export default ChallengesCompletedRanking;

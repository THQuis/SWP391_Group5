...
import { FaCrown, FaSmoking } from "react-icons/fa";
...
function CigarettesDroppedRanking() {
    ...
    useEffect(() => {
        const fetchRanking = async () => {
            try {
                ...
                setLoading(true);
                const response = await fetch(`${process.env.REACT_APP_API_URL}/api/ranking/top-cigarettes-dropped?top=50`, {
                    headers: {
                ...
                if (response.ok) {
                    ...
                    console.log('Cigarettes Dropped Ranking:', data);
                    setRanking(data);
                } else {
    ...
    const maxCigarettes = ranking.length > 0 ? ranking[0].totalCigarettesDropped : 1;
    ...
    const handleExportExcel = () => {
        const data = filteredRanking.map((user, index) => ({
            ...
            "Số điếu thuốc đã bỏ": user.totalCigarettesDropped,
        }));

        const ws = XLSX.utils.json_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Điếu thuốc đã bỏ");

        const excelBuffer = XLSX.write(wb, { bookType: "xlsx", type: "array" });
        saveAs(
            new Blob([excelBuffer], { type: "application/octet-stream" }),
            "bang_xep_hang_dieu_thuoc_da_bo.xlsx"
        );
    };
    ...
    return (
        <>
            <div className="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h4 className="mb-1">🚬 Bảng xếp hạng theo điếu thuốc đã bỏ</h4>
                    <p className="text-muted mb-0">Những người từ bỏ nhiều thuốc lá nhất</p>
                </div>
                    ...
                    <Table hover responsive style={{ background: "transparent" }}>
                        <thead>
                            <tr style={{ backgroundColor: '#f8f9fa' }}>
                                ...
                                <th className="text-center py-3" style={{ width: 180 }}>Điếu thuốc đã bỏ</th>
                            ...
                            {currentPageData.length === 0 ? (
                                ...
                                currentPageData.map((user, index) => {
                                    ...
                                    return (
                                        <tr
                                            key={user.userID}
                                            style={realIdx === 0 ? { background: "#ffebee" } : {}}
                                        >
                                            ...
                                            <td className="text-center py-3">
                                                <div className="d-flex align-items-center justify-content-center gap-2">
                                                    <FaSmoking color="#dc3545" size={18} style={{ transform: 'rotate(45deg)' }} />
                                                    <span className="fw-bold" style={{ fontSize: 18, color: "#dc3545" }}>
                                                        {user.totalCigarettesDropped}
                                                    </span>
                                                    <span className="text-muted">điếu</span>
                                                </div>
                                            </td>
                                            <td className="text-center py-3">
                                                <ProgressBar
                                                    now={Math.min(100, (user.totalCigarettesDropped / maxCigarettes) * 100)}
                                                    variant="danger"
                                                    style={{ height: 8, minWidth: 120 }}
                                                />
                                                <small className="text-muted mt-1 d-block">
                                                    {((user.totalCigarettesDropped / maxCigarettes) * 100).toFixed(1)}%
                                                </small>
                                            </td>
                                        </tr>
                                    );
...
export default CigarettesDroppedRanking;

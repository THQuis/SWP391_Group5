...
const ProgressDashboardPage = () => {
    ...
    const [milestoneData, setMilestoneData] = useState(null); // Thêm state cho milestone data
    const [milestoneError, setMilestoneError] = useState(null); // Thêm state cho lỗi milestone
    ...
    const [timelineSticky, setTimelineSticky] = useState(false);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            setShowBackToTop(scrollTop > window.innerHeight * 0.2);
    ...
    const fetchProgressData = useCallback(async () => {
        try {
            ...
            const timeDiff = startDate ? currentDate - startDate : 0;
    ...
    // Thêm hàm fetchMilestoneData để lấy dữ liệu cột mốc
    ...
    const fetchMilestoneData = useCallback(async () => {
        try {
            ...
            console.log('MockAutomatedProgress health milestones count:', healthMilestones.length);

            console.log('DEBUG: All health milestones before sorting:', healthMilestones.map(m => ({
                id: m.milestoneID,
                userMilestoneID: m.userMilestoneID,
                name: m.milestoneName,
                time: m.milestoneTime,
                unit: m.timeUnit,
            ...
            if (duplicateDetails.length > 0) {
                console.error('🚨 MockAutomatedProgress: Detected API duplicates in health group:', duplicateDetails);
                console.error('🚨 This is a BACKEND API issue - same health milestones returned multiple times');
            }

            // Kiểm tra duplicate milestones
            const duplicates = healthMilestones.reduce((acc, milestone, index) => {
            ...
            if (fullyUniqueMilestones.length !== uniqueMilestones.length) {
                console.warn('MockAutomatedProgress: Removed additional content duplicates in health milestones:', {
                    afterIdFilter: uniqueMilestones.length,
                    afterContentFilter: fullyUniqueMilestones.length,
                    additionalRemoved: uniqueMilestones.length - fullyUniqueMilestones.length
                });
            }

            if (fullyUniqueMilestones.length !== healthMilestones.length) {
                console.warn('MockAutomatedProgress: Total health milestone duplicates removed:', {
                    originalCount: healthMilestones.length,
                    finalUniqueCount: fullyUniqueMilestones.length,
                    totalRemoved: healthMilestones.length - fullyUniqueMilestones.length
                });
            }

            console.log('DEBUG: After removing all duplicates:', fullyUniqueMilestones.length, 'fully unique milestones');
            ...
            // Debug log để kiểm tra thứ tự sắp xếp health milestones
            console.log('🏥 Health milestones sorted by time:',
                sortedMilestones.map(m => ({
                    name: m.milestoneName,
                    time: `${m.milestoneTime} ${m.timeUnit}`,
                    timeInMinutes: parseTimeToMinutes(m.milestoneTime, m.timeUnit),
                    progressPercent: m.progressPercent,
                    completed: m.achieved || m.progressPercent >= 100
                }))
            );

            setMilestoneData(sortedMilestones);
            console.log('Health milestones loaded:', sortedMilestones.length, 'fully unique milestones');
            console.log('Sample milestone data:', sortedMilestones.slice(0, 2));
            ...
            if (completedMilestones.length > 0) {
                ...
                setTimeout(() => {
                    const timelineElement = document.querySelector('.health-timeline-card');
                    if (timelineElement && window.scrollY < timelineElement.offsetTop - 100) {
                        timelineElement.scrollIntoView({
                            behavior: 'smooth',
                            block: 'start',
                            inline: 'nearest'
    ...
    useEffect(() => {
        ...
        const progressInterval = setInterval(fetchProgressData, 60000); // Refresh every 1 minute
        const milestoneInterval = setInterval(fetchMilestoneData, 60000); // Refresh milestones every 1 minute
                                ...
                                <div className="progress-card-content">
                                    ...
                                    <div className="journey-milestones mt-4">
                                        ...
                                        <Row className="g-2">
                                            ...
                                            <Col xs={6}>
                                                <div className={`milestone-badge ${progress.daysSinceStart >= 30 ? 'achieved' : 'pending'}`}>
                                                    <div className="milestone-icon">📅</div>
                                                    <div className="milestone-text">
                                                        <div className="milestone-title">1 tháng</div>
                                                        <div className="milestone-desc">Thay đổi lớn</div>
                                                    </div>
                                                </div>
                                            </Col>
                                            <Col xs={6}>
                                                <div className={`milestone-badge ${progress.daysSinceStart >= 365 ? 'achieved' : 'pending'}`}>
                                                    <div className="milestone-icon">🎉</div>
                                                    <div className="milestone-text">
                                                        <div className="milestone-title">1 năm</div>
                                                        <div className="milestone-desc">Chiến thắng vĩ đại</div>
                                                    </div>
                                                </div>
                                            </Col>
                                        </Row>
